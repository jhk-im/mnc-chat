...
import im.vector.app.features.regeshake.ReportType
        ...
        homeActivityViewModel.observeViewEvents {
            when (it) {
                is HomeActivityViewEvents.AskPasswordToInitCrossSigning -> {
                    //handleAskPasswordToInitCrossSigning(it)
                }
                is HomeActivityViewEvents.OnNewSession                  -> {
                    //handleOnNewSession(it)
                }
                HomeActivityViewEvents.PromptToEnableSessionPush        -> {
                    //handlePromptToEnablePush()
                }
                is HomeActivityViewEvents.OnCrossSignedInvalidated      -> {
                    //handleCrossSigningInvalidated(it)
                }
            }.exhaustive
        }
